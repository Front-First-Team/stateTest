...
function State3() {
  /*
    ...
    1. 구매후기 및 상제를 제외한 데이터의 모든 정보는 화면에 노출되어야 합니다.
        단, 가격표는 3자리마다 ,를 작성해야합니다.  -->  완료

    2. 해당 상품을 클릭하면 상세 페이지로 이동합니다.  -->  완료
    
    3.
      상세페이지 주소에는 클릭한 상품의 상품번호가 노출되어야 합니다
      해당 router (주소설정) 은 제가 모두 app.js에 해두었습니다  -->  path: "/detail/:productNumber"

      ex) /state/detail/301389

      해당 상품번호를 주소에서 부터 받아와야합니다.  -->  { params.productNumber }
      받아온 데이터를 토대로
      ...
      * 주의
      ...
      상세페이지는 pages/Detail.js이며  -->  경로 : /detail/:productNumber
  ...
  */
  ...
  // console.log(productList.products[2]);

  const navigate = useNavigate();

  const onNavigateDetailPage = (pathName) => {
    ...
    <>
      ...
      <ul>
        ...
        {productList.products.map((product) => <ProductCard 
          key={product.productNumber} product={product} onNavigate={onNavigateDetailPage}
        />)}
      </ul>
    </>
  );
